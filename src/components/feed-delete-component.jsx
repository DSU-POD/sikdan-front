...
export default function FeedDeleteComponent({ id, setDeleteOpen }) {
  const router = useRouter();

  const handleDelete = async () => {
    const response = await api.delete(`/feed/delete/${id}`);
  ...
  return (
    <Dialog open={true} onClose={setDeleteOpen} className="relative z-10">
      <DialogBackdrop
        transition
        className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in"
      />

      <div className="fixed bottom-20 inset-0 z-10 w-screen overflow-y-auto">
